...
void near_cup(){
	...
	while(1)	//infinite loop
	{
		if(analog10(ANALOG_ET_SENSOR_PORT) > 512) //is close
		{
			...
			close_clippers();
			bring_servo_up();
			//TO DO: move forward/backward and open clippers
		}
...
void bring_servo_up(){
	...
	set_servo_position(CLIPPER_UP_DOWN_PORT,0);   //(port,angle)
	disable_servos();
...
void bring_servo_down(){
	...
	set_servo_position(CLIPPER_UP_DOWN_PORT,2013);
	disable_servos();
}

void close_clippers(){
	//TO BE UPGRADED: the num const could differ
	enable_servos();
	set_servo_position(CLIPPER_UP_DOWN_PORT,CLOSE_CLIPPERS);
        printf("Close");
	disable_servos();
}
...
void open_clippers(){
	enable_servos();
	//TO BE TESTED
	set_servo_position(CLIPPER_OPEN_CLOSE_PORT,190);
	disable_servos();
}
